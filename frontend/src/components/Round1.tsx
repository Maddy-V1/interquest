...
function Round1() {
  ...
  const checkRoundStatus = async () => {
    try {
      ...
      const response = await fetch('/api/admin/round-status/1')
      if (response.ok) {
        const data = await response.json()
  ...
  const handleStartRound1 = async () => {
    ...
    if (roundStatus !== 'active') {
      alert('Round 1 is currently not available. Please wait for admin to start the round.')
      return
    }
              ...
              {isCompleted ? (
                ...
                <div className="space-y-4">
                  {roundStatus === 'stopped' && (
                    <div className="bg-red-50 border-2 border-red-200 rounded-xl p-6 mb-4">
                      <div className="flex items-center justify-center gap-3">
                        ...
                        <div>
                          <h3 className="text-2xl font-bold text-red-800">Round 1 Not Available</h3>
                          <p className="text-red-600">Please wait for admin to start Round 1</p>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  <button
                    ...
                    disabled={isLoading || roundStatus !== 'active' || statusLoading}
                    className={`px-12 py-4 rounded-xl text-xl font-bold transition-all duration-300 transform ${
                      roundStatus === 'active' 
                        ? 'bg-gradient-to-r from-blue-500 to-purple-600 text-white hover:-translate-y-1 hover:shadow-2xl hover:shadow-blue-500/30' 
                  ...
                  >
                    {statusLoading ? (
                      ...
                      <>
                        {roundStatus === 'active' ? 'Start Round 1' : 'Round 1 Unavailable'}
                        {roundStatus === 'active' && <span className="ml-2">→</span>}
                      </>
                    )}
                  </button>
                </div>
